...
const char *FtpInternal::ftpResponse(int iOffset)
{
    ...
    if (iOffset < 0) {
        ...
        do {
            ...
            m_lastMultilineResponse.append(QString::fromUtf8(m_lastControlLine).trimmed()); // Store each line
...
Result FtpInternal::ftpLogin(bool *userChanged)
{
    ...
    // Query server features like MDTM after successful login
    ftpQueryFeatures();
...
// New function to query and parse server features
void FtpInternal::ftpQueryFeatures()
{
    ...
    if (!ftpSendCmd(QByteArrayLiteral("FEAT"))) {
        qCDebug(KIO_FTPS) << "FEAT command failed, assuming no extended features.";
        return;
    }

    if (m_iRespType != 2) {
        qCDebug(KIO_FTPS) << "FEAT command not supported or failed with response type" << m_iRespType;
        return;
    }

    // Iterate through the collected multiline response to find "MDTM"
...
int FtpInternal::ftpOpenPortDataConnection()
{
    ...
    if (!m_server) {
        ...
        m_server->setProxy(QNetworkProxy::DefaultProxy);
...
Result FtpInternal::ftpOpenCommand(const char *_command, const QString &_path, char _mode, int errorcode, KIO::fileoffset_t _offset)
{
    ...
    if (!ftpSendCmd(tmp) || (m_iRespType != 1)) {
        ...
        if (useDataEnc) {
            ...
            if (result != 0) {
                qCDebug(KIO_FTPS) << "TLS Negotiation failed on the data channel.";
                return Result::fail();
            }
        }
        ...
        if (m_data) {
            qCDebug(KIO_FTPS) << "connected with remote.";
            m_bBusy = true; // cleared in ftpCloseCommand
            return Result::pass();
        }

        qCDebug(KIO_FTPS) << "no connection received from remote.";
...
bool FtpInternal::ftpChmod(const QString &path, int permissions)
{
    ...
    if (ftpSendCmd(cmd)) {
        qCDebug(KIO_FTPS) << "ftpChmod: Failed to issue chmod";
        return false;
    }
...
Result FtpInternal::ftpStatAnswerNotFound(const QString &path, const QString &filename)
{
    ...
    qCDebug(KIO_FTPS) << "statSide=" << statSide;
...
{
    qCDebug(KIO_FTPS) << "path=" << url.path();
    auto result = ftpOpenConnection(LoginMode::Implicit);
    if (!result.success()) {
        return result;
    }
    ...
    qCDebug(KIO_FTPS) << "cleaned path=" << path;
    ...
    qCDebug(KIO_FTPS) << "details=" << details;
    if (details == 0) {
    ...
    while (ftpReadDir(ftpEnt)) {
        ...
        if (!bFound) {
            if (filename == ftpEnt.name && !filename.isEmpty()) {
                bFound = true;
                if (!isDir) { // Only try MDTM for files, not directories
                    if (m_mdtmSupported) { // Only try MDTM if the server advertises support for it
                        const QDateTime precise = ftpMdtm(path);
                        if (precise.isValid()) {
                            ftpEnt.date = precise;
                        }
                    }
...
Result FtpInternal::listDir(const QUrl &url)
{
    ...
    if (path.isEmpty()) {
        ...
        realURL.setPath(m_initialPath);
        qCDebug(KIO_FTPS) << "REDIRECTION to " << realURL;
        q->redirection(realURL);
        return Result::pass();
    }
...
bool FtpInternal::ftpReadDir(FtpEntry &de)
{
    ...
    while (true) {
        ...
        if (m_usingMlsd) {
            ...
            // Example: Type=file;Size=123;Modify=20241022143000; my file.txt
            ...
            if (de.name == QLatin1String(".") || de.name == QLatin1String("..")) continue;
            ...
            for (const QString &fact : facts) {
                int eqIdx = fact.indexOf(QLatin1Char('='));
                if (eqIdx == -1) continue;
                QString name = fact.left(eqIdx).toLower();
                QString value = fact.mid(eqIdx + 1);

                if (name == QLatin1String("type")) {
                    if (value.startsWith(QLatin1String("dir"))) de.type = S_IFDIR;
                    else if (value.startsWith(QLatin1String("os.unix=slink"))) {
                        // Handle symlinks if the server provides unix facts
                    }
...
Result FtpInternal::ftpGet(int iCopyFile, const QString &sCopyFile, const QUrl &url, KIO::fileoffset_t llOffset)
{
    ...
    if (!ftpSize(url.path(), '?') && (m_iRespCode == 550) && ftpFolder(url.path())) {
        ...
        qCDebug(KIO_FTPS) << "it is a directory in fact";
    ...
    if (!resumeOffset.isEmpty()) {
        ...
        qCDebug(KIO_FTPS) << "got offset from metadata : " << llOffset;
    }
    ...
    qCDebug(KIO_FTPS) << "starting with offset=" << llOffset;
...
bool FtpInternal::ftpDataMode(char cMode)
{
    ...
    qCDebug(KIO_FTPS) << "want" << cMode << "has" << m_cDataMode;
...
bool FtpInternal::ftpFolder(const QString &path)
{
    ...
    qCDebug(KIO_FTPS) << "want" << newPath << "has" << m_currentPath;
...
Result FtpInternal::ftpCopyGet(int &iCopyFile, const QString &sCopyFile, const QUrl &url, int permissions, KIO::JobFlags flags)
{
        ...
        if (result.success()) {
            ...
            if (!QFile::rename(sPart, sCopyFile)) {
                ...
                if (!bDestExists || !(QFile::remove(sCopyFile) && QFile::rename(sPart, sCopyFile))) {
                    qCDebug(KIO_FTPS) << "cannot rename " << sPart << " to " << sCopyFile;
    ...
    if (result.success()) {
        ...
        if (!mtimeStr.isEmpty()) {
            ...
            if (dt.isValid()) {
                qCDebug(KIO_FTPS) << "Updating modified timestamp to" << mtimeStr;
...
Result FtpInternal::ftpSendMimeType(const QUrl &url)
{
    ...
    if (!buffer.isEmpty()) {
        ...
        qCDebug(KIO_FTPS) << "Emitting MIME type" << mime.name();
...
void FtpInternal::fixupEntryName(FtpEntry *e)
{
    ...
    if (e->type == S_IFDIR) {
        if (!ftpFolder(e->name)) {
            ...
            if (ftpFolder(name)) {
                e->name = name;
                qCDebug(KIO_FTPS) << "fixing up directory name from" << e->name << "to" << name;
            } else {
                int index = 0;
        ...
        if (!ftpFileExists(e->name)) {
            ...
            if (ftpFileExists(name)) {
                e->name = name;
                qCDebug(KIO_FTPS) << "fixing up filename from" << e->name << "to" << name;
            } else {
                int index = 0;
                while (e->name.at(index).isSpace()) {
                    index++;
...
ConnectionResult FtpInternal::synchronousConnectToHost(const QString &host, quint16 port)
{
    ...
    if (socketError == QAbstractSocket::ProxyAuthenticationRequiredError) {
        ...
        if (!q->checkCachedAuthentication(info)) {
            ...
            if (errorCode != KJob::NoError) {
                qCDebug(KIO_FTPS) << "user canceled proxy authentication, or communication error." << errorCode;
